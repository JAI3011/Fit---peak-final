...
import { X, Save, Utensils, Loader } from 'lucide-react';
import { useTrainer } from '../../context/TrainerContext';

const SelectDietModal = ({ isOpen, onClose, onAssign }) => {
  const { dietPlans } = useTrainer();
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <select
                  ...
                  required
                >
                  <option value="" disabled className="bg-zinc-900">Choose a diet plan...</option>
                  {dietPlans.length > 0 ? (
                    dietPlans.map(plan => (
                ...
                {dietPlans.length === 0 && (
                  <p className="text-xs text-amber-400 mt-2">
                    You haven't created any diet templates yet.
                  </p>
                )}
              </div>
              ...
              {selectedPlanId && (
                <div className="p-4 bg-white/5 rounded-xl border border-white/5">
                  <p className="text-sm text-zinc-400">
                    {dietPlans.find(p => p.id === selectedPlanId)?.description}
                  </p>
                </div>
              )}
